...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    <div class=" overflow-hidden rounded-lg mx-4">
      <img
        src="./images/events/${events[i].imageUrl}"
        alt="image"
        class="w-full mt-5 rounded-xl shadow-xl backdrop-blur-xl"
      />
      <div class="p-8 text-center sm:p-9 md:p-7 xl:p-9">
        ...
        <h3
            class="text-dark text-[var(--blue)] rounded-sm w-full bg-[var(--white)] bg-opacity-10 hover:text-primary mb-4 block text-xl font-semibold sm:text-[22px] md:text-xl lg:text-[22px] xl:text-xl 2xl:text-[22px]"
          >
          ${events[i].eventDate}
